...
> This project is an independent derivative of LangBot and is not an official LangBot distribution.

**A LangBot-based Feishu notification gateway, message router, and Agent invocation platform.**

## What this project changes

Feishu Agent Hub keeps LangBot's bot and Agent foundation while adding the capabilities needed to use Feishu as a programmable notification channel and Agent entry point:

- **Managed notification center**: Save Feishu direct-message and group conversations reachable by a bot as reusable targets. The admin UI, HTTP API, and MCP can send to one or many targets; idempotency keys prevent duplicate jobs, and per-target delivery results are retained.
- **Explicit inbound routing**: A bot can operate in routes-only mode. Messages without a matching route are recorded but never invoke an Agent. Group routes independently support mention-only and all-matching-message triggers.
- **External Agent connectors**: Administrators configure an Agent endpoint, system prompt, allowed skills, and MCP endpoint. Each invocation includes recent history plus messages not yet acknowledged by that Agent, and the context cursor advances only after an explicit acceptance response.
- **Agent-oriented API and MCP**: Agents can discover approved notification targets, send notifications, inspect delivery jobs, and read routing resources. The admin UI includes a copy-ready Agent Guide and MCP client configurations for Codex and other compatible Agents.

> The original LangBot overview and usage documentation remain below to make upstream synchronization easier. See [`docs/specs/notification-gateway.md`](docs/specs/notification-gateway.md) for the Feishu Agent Hub design.
