...
Important tools currently include list_notification_targets, send_notification, get_notification_job, list_bots, send_message, list_pipelines, get_pipeline, list_skills, and knowledge-base inspection tools. Tool schemas returned by MCP are authoritative if this text and the live server differ.`,
  ...
  agentConnectors: {
    ...
    description:
      'Manage external Agents or a local Codex Bridge that receives routed Feishu messages.',
    refresh: 'Refresh',
    create: 'Add connector',
    edit: 'Edit connector',
    delete: 'Delete connector',
    managed: 'Managed connectors',
    name: 'Name',
    kind: 'Type',
    ...
    disabled: 'Disabled',
    empty:
      'No Agent connectors yet. Add one, then select it in a bot routing rule.',
    conversationHistory: 'Conversation history and invocation audit',
    noConversations:
      'Conversation history appears here after a connector is invoked by a route.',
    viewHistory: 'View conversation history',
    messages: 'Messages',
    invocationAudit: 'Invocation audit',
    kinds: { http: 'Standard HTTP Agent', codex_bridge: 'Codex Bridge' },
    protocolTitle: 'Connector protocol',
    protocolDescription:
      'After a route matches, the gateway POSTs the system prompt, allowed skills, MCP endpoint, recent history, and every unacknowledged message. The cursor advances only when the endpoint returns accepted: true.',
    securityHint:
      'API keys are never stored here. Public endpoints must use HTTPS; loopback HTTP is allowed for 127.0.0.1, localhost, and ::1.',
    formDescription:
      'A connector hands the standard protocol to your Agent runtime. Codex Bridge labels that use case but does not execute local commands directly.',
    timeout: 'Timeout in seconds',
    endpointHint:
      'Use HTTPS or loopback HTTP. The URL must not contain embedded credentials.',
    systemPrompt: 'Agent system prompt',
    skillNames: 'Allowed skill names',
    skillNamesPlaceholder: 'Separate multiple skills with commas',
    enabledHint:
      'A disabled connector is never called, even if a route matches.',
    requiredFields: 'Name and endpoint URL are required.',
    loadError: 'Failed to load Agent connectors: {{error}}',
    historyLoadError: 'Failed to load Agent conversation history: {{error}}',
    saveError: 'Failed to save Agent connector: {{error}}',
    deleteError: 'Failed to delete Agent connector: {{error}}',
    createSuccess: 'Agent connector created',
    updateSuccess: 'Agent connector updated',
    deleteSuccess: 'Agent connector deleted',
    deleteConfirmation:
      'Delete “{{name}}”? Bot routes that reference it will fail closed and will not fall back to another Agent.',
  },
  bots: {
    title: 'Bots',
    description:
    ...
    selectRouteDestination: 'Select pipeline or Agent',
    routeDestinationRequired:
      'Each rule must select exactly one pipeline or Agent connector',
