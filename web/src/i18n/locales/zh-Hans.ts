...
当前重要工具包括 list_notification_targets、send_notification、get_notification_job、list_bots、send_message、list_pipelines、get_pipeline、list_skills 和知识库查询工具。如果本说明与 MCP 实时返回的工具结构不一致，以 MCP 工具结构为准。`,
  ...
  agentConnectors: {
    title: 'Agent 连接器',
    description: '管理接收飞书入站消息的外部 Agent 或本机 Codex Bridge。',
    refresh: '刷新',
    create: '添加连接器',
    edit: '编辑连接器',
    delete: '删除连接器',
    ...
    enabled: '已启用',
    disabled: '已停用',
    empty: '还没有 Agent 连接器。添加后，可在机器人路由规则中选择它。',
    conversationHistory: '会话历史与调用审计',
    noConversations: '连接器被路由调用后，会话历史会显示在这里。',
    viewHistory: '查看会话历史',
    messages: '消息记录',
    invocationAudit: '调用审计',
    kinds: { http: '标准 HTTP Agent', codex_bridge: 'Codex Bridge' },
    protocolTitle: '连接协议',
    protocolDescription:
      '命中路由后，系统会 POST 系统提示、允许的技能、MCP 地址、最近历史和所有未确认消息；只有接收端返回 accepted: true 才推进上下文游标。',
    securityHint:
      '本页面不保存 API 密钥。公网接收端必须使用 HTTPS；本机服务可使用 127.0.0.1、localhost 或 ::1 的 HTTP 地址。',
    formDescription:
      '连接器负责把统一协议交给你的 Agent 运行时。Codex Bridge 是同一协议的专用标记，不会直接执行本机命令。',
    timeout: '超时秒数',
    endpointHint: '仅允许 HTTPS，或本机回环地址上的 HTTP；URL 不得内嵌凭据。',
    systemPrompt: 'Agent 系统提示',
    skillNames: '允许的技能名称',
    skillNamesPlaceholder: '多个技能用英文逗号分隔',
    enabledHint: '停用后，即使命中路由也不会调用该连接器。',
    requiredFields: '名称和接收端 URL 为必填项。',
    loadError: '加载 Agent 连接器失败：{{error}}',
    historyLoadError: '加载 Agent 会话历史失败：{{error}}',
    saveError: '保存 Agent 连接器失败：{{error}}',
    deleteError: '删除 Agent 连接器失败：{{error}}',
    createSuccess: 'Agent 连接器已创建',
    updateSuccess: 'Agent 连接器已更新',
    deleteSuccess: 'Agent 连接器已删除',
    deleteConfirmation:
      '确定删除“{{name}}”吗？引用它的机器人路由会保持失败关闭，不会回退到其他 Agent。',
  },
  bots: {
    title: '机器人',
    ...
    selectRouteDestination: '选择流水线或 Agent',
    routeDestinationRequired: '每条规则必须且只能选择一个流水线或 Agent 连接器',
    selectBot: '请选择机器人',
    botLogTitle: '机器人日志',
